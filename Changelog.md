...
- nPM1300 and nPM1304: New Fuel Gauge tab, which includes the old Fuel Gauge
  feature and a new Battery Health tile.
...
- Enabled LDO SoftStart for nPM1304.

### Fixed

- nPM2100: Issue with setting the timer period value from the UI.
- nPM2100: When the GPIO mode control is selected, the toggle for Load
  switch/LDO is now correctly enabled.
- nPM1300 and nPM1304: Fixed SoftStart currents values.
...
- nPM2100: Setting the Boost regulator to the forced pass-through mode will now
  first set the output voltage VOUT to the minimum setting (1.8 V), in
...
- nPM1300:
    - Battery current limits (IBATLIM) have been lowered. High is now set at
      1000 mA (previously 1370 mA) and low is now set at 200 mA (previously 270
      mA).
...
- Issue in the firmware for nPM1304 where battery measurements would stop while
  the battery was charging.
- Issue with the battery status showing `iBat` resolution incorrectly. It is now
  using one decimal point for nPM1304 as intended.
...
- Experimental battery profiling for the nPM1304 EK with hardware revision newer
  than 0.1.0.
...
- The default view at the application start is now a welcome screen without any
  device-specific tabs, which are now added and removed based on the selected
  device.
...
- Pre-bundled list of the Renata battery models that can be uploaded to the nPM
  1300 EK. These models can be found in the side panel under **Add new active
  battery model**.
...
- The Lipol battery models are no longer part of the firmware, and will now need
  to be uploaded to the nPM 1300 EK. These models can be found in the side panel
  under **Add new active battery model**.

### Fixed

- Profiling a battery with no NTC will now save the user temperature to file
  instead of saving the fallback value of 25°C.
- Tooltip for the **NTC thermistor** when configuring a profile. The tooltip is
  now visible.
...
- Removed overlay properties for thermistor-cold-millidegrees,
  ...
  thermistor-hot-millidegrees as conversion from millidegrees to register values
  is incorrect, resulting in charging being disabled incorrectly.
...
- Corrected an issue where the 'Restart application with verbose logging' button
  failed to restart the application.
- Rectified a problem where the 'Abort' command during profiling would not close
  the dialog if the device was disconnected.
- Resolved an issue causing the application to crash if a profiling project was
  deleted while the application was running.
